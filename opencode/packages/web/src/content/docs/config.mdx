...
- `verify_commands` - commands to run instead of auto-detecting them. Useful in monorepos, where only root scripts are detected.
...
- `vibe_review` - the planner reviews each executor step (using the transcript and the current contents of the step's target files) and either approves it or sends it back with feedback. Defaults to `true`.
