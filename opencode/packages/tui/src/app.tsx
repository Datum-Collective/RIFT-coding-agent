...
import { getSessionStripItems, SessionStatusStrip } from "./component/session-status-strip"
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "home") {
      renderer.setTerminalTitle("RIFT")
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`RIFT | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`RIFT | ${route.data.id}`)
    }
  })
  ...
  const blockedSessions = createMemo(() => {
    const currentSessionID = route.data.type === "session" ? route.data.sessionID : undefined
    return getSessionStripItems(sync.data, sync.data.session, currentSessionID).filter(
      (session) => session.status === "blocked_waiting_for_input",
    )
  })
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Session",
        suggested: sync.data.session.length > 1,
        slashName: "mission",
        run: () => {
      ...
      {
        name: "session.blocked.next",
        title: "Jump to next blocked session",
        category: "Session",
        hidden: true,
        run: () => {
          ...
          const currentSessionID = route.data.type === "session" ? route.data.sessionID : undefined
          const currentIndex = blocked.findIndex((session) => session.id === currentSessionID)
          const next = blocked[(currentIndex + 1) % blocked.length]
          if (next) route.navigate({ type: "session", sessionID: next.id })
        },
      },
      {
      ...
      },
      {
        ...
        run: () => {
          local.agent.move(1)
          if (local.agent.current()?.name === "vibe" && !local.agent.vibe.configured()) {
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to RIFT v${result.data.version}. Please restart the application.`,
    )
