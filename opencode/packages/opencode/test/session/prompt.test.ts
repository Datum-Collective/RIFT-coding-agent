...
function vibeProviderCfg(url: string) {
  const models = cfg.provider.test.models
  return {
    ...providerCfg(url),
    ...
    provider: {
      ...
      test: {
        ...
        models: {
          ...models,
          "planner-model": { ...models["test-model"], id: "planner-model", name: "Planner Model" },
          "executor-model": { ...models["test-model"], id: "executor-model", name: "Executor Model" },
        },
      },
    },
  }
...
it.instance("vibe mode uses planner and executor models for separate turns", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text(
      '{"steps":[{"files":["src/example.ts"],"functions":["run"],"description":"Update run"},{"files":["src/example.test.ts"],"functions":["run test"],"description":"Add coverage"}]}',
    )
    yield* llm.text("executor complete step one")
    yield* llm.text("executor complete step two")
    ...
    expect(hits[0]?.body.model).toBe("planner-model")
