...
import type { BrowserOpenTool } from "@/tool/browser_open"
...
type ToolDefs = {
  ...
  browser_open: typeof BrowserOpenTool
...
function runBrowserOpen(p: ToolProps<typeof BrowserOpenTool>): ToolInline {
  const url = p.input.url ?? ""
  return {
    icon: "◉",
    title: url ? `BrowserOpen ${url}` : "BrowserOpen",
  }
}
...
function scrollBrowserOpenStart(p: ToolProps<typeof BrowserOpenTool>): string {
  const url = p.input.url ?? ""
  if (!url) {
    return "◉ BrowserOpen"
  }
...
function permBrowserOpen(p: ToolPermissionProps<typeof BrowserOpenTool>): ToolPermissionInfo {
  const url = p.input.url || ""
  return {
    icon: "◉",
...
const TOOL_RULES = {
  ...
  browser_open: {
    ...
    run: runBrowserOpen,
    scroll: {
      start: scrollBrowserOpenStart,
    },
    permission: permBrowserOpen,
  },
  websearch: {
    view: {
      output: false,
