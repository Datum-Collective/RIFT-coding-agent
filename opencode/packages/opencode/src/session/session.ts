...
export interface Interface {
  ...
  readonly setVibeMode: (input: { sessionID: SessionID; enabled: boolean }) => Effect.Effect<void, NotFound>
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const setVibeMode = Effect.fn("Session.setVibeMode")(function* (input: {
      sessionID: SessionID
      enabled: boolean
    }) {
      ...
      yield* setMetadata({
        ...
        metadata: {
          ...(current.metadata ?? {}),
          [VIBE_MODE_METADATA_KEY]: input.enabled,
        },
      })
    })
