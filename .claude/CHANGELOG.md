...
### New
- **Replay and time travel for everything RIFT changes.** Every step that changes files is committed onto `refs/rift/history`, with the pre-RIFT tree as the base and a separate "Outside RIFT" commit for hand edits between steps. `rift replay <session>` lists each step's decision, tool calls, files, checks judged by exit code, failures, repairs and final evidence; `rift replay` alone shows the activity grid. `/replay` in the TUI shows the same grid, opens a turn, and travels back through session revert (`/redo` undoes the trip).

### Changed
- **Engineering graph: checks replace tests, claims dropped.** Graph nodes track checks instead of tests, the claims concept is removed, and a new evidence module backs the graph. Sidebar, task util, and generated schema/SDK types updated alongside.

## v0.1.12 — let the plant keep growing! (2026-09-23)

### Fixed
- **Verify outside the project.** When every edited file sits outside the project directory (e.g. a second worktree), checks run where the files actually are instead of the wrong project. Files with no git history render as current contents so they still get checked.

### New
- **Browser check over `file://`.** The browser tool opens local pages over `file://` under the same read permission as the read tool. With no verify URL configured, the check opens the page the turn built (`index.html` first, else the first edited HTML file). A missing HTTP status on `file://` pages no longer reads as a failed load.
- **Engineering graph.** Per-session graph nodes (status, owner, dependencies, files, tests, decisions, evidence) stored in sqlite, updated through graph tools, listed over the API, shown in the sidebar with a footer link to the graph view. Schema and generated types updated alongside.

## v0.1.11 (2026-09-23)

### Fixed
- **Vibe Mode on Zen Responses models.** Vibe's internal planner, reviewer, revision, and claims calls no longer send unsupported static `tool_choice` values to the Zen Responses transport (e.g. Muse Spark 1.3 Free). Tool-choice handling is now capability-aware and scoped to the Responses transport.
- **Windows installer.** Architecture detection is robust (`PROCESSOR_*` env vars) with self-verification of PATH and install.

### Changed
- README download links are platform-aware (Yatko badge).

## v0.1.10 — Free Vibe (2026-09-21)

### Fixed
- **Vibe Mode with free-tier models.** Vibe planner and reviewer calls use the proper SessionTools definitions, with fallback handling for non-JSON planner responses. Verified end-to-end with Big Pickle planner + Ling 3.0 Flash Fin Free executor on Windows.
